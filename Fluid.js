class Particle {
	...
	solve(particles, RADIUS_TERM, e, sVISCOSITY) {
	...
	static fluidSim(particles, MIN_BOUND_X, MIN_BOUND_Y, MAX_BOUND_X, MAX_BOUND_Y, RADIUS = 3, e = 0, VISCOSITY = 0.1) {
		...
		function update(solves = 1) {
			...
			for (let n = 0; n < solves; n++) for (let i = 0; i < particles.length; i++) {
				let p = particles[i];
				particles[i].wallResolve(e, VISCOSITY);
				let { x, y } = getGrid(p);
				p.solve(particleGrid[x][y], RADIUS_TERM, e, VISCOSITY);
			}
			for (let i = 0; i < particles.length; i++) {
				particles[i].wallResolve(e, VISCOSITY);
			}
